...
const valorantGroupData: Record<"A" | "B" | "C", SimpleGroupRow[]> = {
  A: [
    { rank: 1, team: "Gen.G", points: 230 },
    { rank: 2, team: "FNATIC", points: 210 },
    { rank: 3, team: "Sentinels", points: 190 },
  ],
  B: [
    { rank: 1, team: "T1", points: 225 },
    { rank: 2, team: "Cloud9", points: 205 },
    { rank: 3, team: "FaZe Clan", points: 185 },
  ],
  C: [
    { rank: 1, team: "Loud", points: 220 },
    { rank: 2, team: "PRX", points: 200 },
    { rank: 3, team: "Acend", points: 180 },
  ],
};
...
const getGroupRowsForGame = (
  ...
  valorantGroups?: Record<"A" | "B" | "C", SimpleGroupRow[]>
): SimpleGroupRow[] => {
  if (gameId === "ml") {
  ...
  if (gameId === "valorant" && (group === "A" || group === "B" || group === "C")) {
    const sourceGroups = valorantGroups ?? valorantGroupData;
    const baseRows = (sourceGroups[group] ?? valorantGroupData[group]).map((row, index) => ({
      ...row,
      originalIndex: row.originalIndex ?? index,
    }));
...
const EventLeaderboard = () => {
  ...
  // Valorant group state (3 groups: A, B, C)
  const [valorantGroupsData, setValorantGroupsData] = useState<Record<"A" | "B" | "C", SimpleGroupRow[]>>(() => ({
    A: valorantGroupData.A.map((row, index) => ({ ...row, originalIndex: row.originalIndex ?? index })),
    B: valorantGroupData.B.map((row, index) => ({ ...row, originalIndex: row.originalIndex ?? index })),
    C: valorantGroupData.C.map((row, index) => ({ ...row, originalIndex: row.originalIndex ?? index })),
  }));

  // Mobile Legends bracket state (initialized from group results)
  const [mlBracket, setMlBracket] = useState<Bracket | null>(null);

  // Valorant bracket state
  ...
  useEffect(() => {
    const hasStarted = !!valorantBracket && valorantBracket.columns.some(col => col.matches.some(m => m.status !== 'upcoming' || m.scoreA !== 0 || m.scoreB !== 0));
    if (!valorantBracket || !hasStarted) {
      setValorantBracket(buildValorantBracketFromGroups());
    }
  }, [valorantGroupsData]);
  ...
  const handleValorantScoreChange = (col: number, mIdx: number, newA: number, newB: number) => {
    setValorantBracket((prev) => {
      if (!prev) return prev;
      const next = { ...prev, columns: prev.columns.map(c => ({ ...c, matches: c.matches.map(m => ({ ...m })) })) } as Bracket;
      next.columns[col].matches[mIdx].scoreA = newA;
      next.columns[col].matches[mIdx].scoreB = newB;
  ...
  const updateValorantTeamName = (group: "A" | "B" | "C", origIdx: number, name: string) => {
  ...
  const updateValorantPoints = (group: "A" | "B" | "C", origIdx: number, value: string) => {
    const points = Number(value) || 0;
    setValorantGroupsData((prev) => {
      const next = { ...prev };
      const rows = next[group] ? [...next[group]] : [];
    ...
    const loadValorant = async () => {
      ...
      try {
        ...
        if (snapshot?.groups) {
          ...
          setValorantGroupsData(base);
          try {
            const snapAny = snapshot as any;
            if (snapAny?.bracket?.columns) {
              setValorantBracket(snapAny.bracket as Bracket);
            } else {
              setValorantBracket(buildValorantBracketFromGroups());
            }
          } catch {
  ...
  const saveAllPoints = async () => {
    try {
      ...
      if (gameId === "bgmi") {
        ...
        const payload = { groups: valorantGroupsData, bracket: valorantBracket ?? buildValorantBracketFromGroups() } as any;
        await savePointsSnapshot(eventId, gameId, payload);
      } else {
        return;
      }
                  ...
                  <Card className="glass-card border-primary/20">
                    <CardHeader>
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        {canEdit && (gameId === 'bgmi' || gameId === 'ml' || gameId === 'freefire' || gameId === 'valorant') && (
                          <div className="flex items-center gap-3 text-sm">
                            {isSupabaseConfigured() ? (
                              <>
                                <div className="text-muted-foreground">
                          ...
                          <Table>
                            ...
                            <TableBody>
                              {gameId === 'bgmi' ? (
                                ...
                                getGroupRowsForGame(gameId, "A", mlGroupData, valorantGroupsData).map((row) => {
                                      ...
                                      <TableCell>
                                        <div className="flex items-center gap-3">
                                          ...
                                          {isMobileLegends && canEdit ? (
                                            ...
                                            <Input value={displayTeam} onChange={(e) => updateValorantTeamName('A', origIdx, e.target.value)} />
                                          ) : (
                                            <div>{displayTeam}</div>
                                          )}
                                        </div>
                          ...
                          <Table>
                            ...
                            <TableBody>
                              {gameId === 'bgmi' ? (
                                ...
                                getGroupRowsForGame(gameId, "B", mlGroupData, valorantGroupsData).map((row) => {
                                  const { gamesPlayed, gamesWon } = deriveGroupRowStats(row);
                                  const origIdx = row.originalIndex ?? row.rank - 1;
                                  const avatar = "B";
                                      ...
                                      <TableCell>
                                        <div className="flex items-center gap-3">
                                          ...
                                          {isMobileLegends && canEdit ? (
                                            ...
                                            <Input value={displayTeam} onChange={(e) => updateValorantTeamName('B', origIdx, e.target.value)} />
                                          ) : (
                                            <div>{displayTeam}</div>
                                          )}
                                        </div>
                          ...
                          <Table>
                            ...
                            <TableBody>
                              {gameId === 'bgmi' ? (
                                ...
                                getGroupRowsForGame(gameId, "C", mlGroupData, valorantGroupsData).map((row) => {
                                  const { gamesPlayed, gamesWon } = deriveGroupRowStats(row);
                                  const origIdx = row.originalIndex ?? row.rank - 1;
                                  const avatar = "C";
                                      ...
                                      <TableCell>
                                        <div className="flex items-center gap-3">
                                          ...
                                          {isMobileLegends && canEdit ? (
                                            ...
                                            <Input value={displayTeam} onChange={(e) => updateValorantTeamName('C', origIdx, e.target.value)} />
                                          ) : (
                                            <div>{displayTeam}</div>
                                          )}
                                        </div>
...
{gameId !== 'freefire' && gameId !== 'bgmi' && (
        ...
        return a.team.localeCompare(b.team);
              ...
              <div className="space-y-6">
                <h3 className="font-orbitron text-2xl tracking-tight">Knockout Stage — Group Winners</h3>
                {canEdit && (
                  <div className="flex items-center gap-3 text-sm mt-2">
                    {isSupabaseConfigured() ? (
                      <>
                        ...
                        <Button size="sm" variant="outline" onClick={() => { setValorantBracket(buildValorantBracketFromGroups()); setIsDirty(true); toast.success('Bracket reseeded from groups'); }}>Reseed from Groups</Button>
                        <Button size="sm" disabled={!isDirty || savingPoints} onClick={saveAllPoints}>{savingPoints ? "Saving..." : "Save"}</Button>
                      </>
                    ) : (
                      <div className="text-red-500">Supabase not configured</div>
                ...
                <div className="rounded-lg border border-border/40 bg-gradient-to-b from-background/60 to-background/30 p-4">
                  <BracketView bracket={(valorantBracket ?? buildValorantBracketFromGroups()) as any} onScoreChange={canEdit ? handleValorantScoreChange : undefined} />
                </div>
              </div>
            ) : (
              <div className="space-y-6">
